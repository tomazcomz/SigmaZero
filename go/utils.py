...
def _get_captured_territories(self,i,j,board,ct_group=set(),captor=0,visited=set()):
    if (i,j) in visited or invalid_position(i,j,len(board)):
        return ct_group, captor
    visited.add((i,j))
    if board[i][j] != 0:    # if this position isn't empty, it checks whose player it belongs to
        if captor == 0:
            captor = board[i][j]    # getting the captor of this group.
            return ct_group, captor
    ...
    # if this position is empty, then ... (continue from here)
